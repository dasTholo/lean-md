# Visual Companion (lmd companion — browser-based mockup/diagram brainstorming, just-in-time)
...
#           "screen_dir":"/path/to/project/.lean-ctx/lean-md/brainstorm/12345-1706000000/content",
#           "state_dir":"/path/to/project/.lean-ctx/lean-md/brainstorm/12345-1706000000/state"}
```
...
**Finding connection info:** The server writes its startup JSON to `$STATE_DIR/server-info`. If you launched the server in the background and didn't capture stdout, read that file to get the URL and port. When using `--project-dir`, check `<project>/.lean-ctx/lean-md/brainstorm/` for the session directory.

**Note:** Pass the project root as `--project-dir` so mockups persist in `.lean-ctx/lean-md/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.lean-ctx/` to `.gitignore` if it's not already there.
...
If the session used `--project-dir`, mockup files persist in `.lean-ctx/lean-md/brainstorm/` for later reference. Only `/tmp` sessions get deleted on stop.
