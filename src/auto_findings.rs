//! Auto-findings extractor — vendored from lean-ctx `core/auto_findings.rs`.
//! Extracts a concise session finding from a lean-ctx tool call result.
//! Pure (regex/parse + Mutex dedup), no external state, no lean-ctx core dep.
use std::sync::Mutex;
...
fn is_noise_path(path: &str) -> bool {
    ...
    // Home dotfiles via stdlib (no dirs dep).
    if let Ok(home) = std::env::var("HOME") {
        let home_s = home.replace('\\', "/");
...
mod tests {
    ...
    fn noise_path_detects_home_dotfiles() {
        if let Ok(home) = std::env::var("HOME") {
            let ssh = format!("{home}/.ssh/config");
